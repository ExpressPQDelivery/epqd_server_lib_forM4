...
*/
		...
		#include <wolfcrypt/postquantum/dilithium/api_dilithium.h>
	#endif
	#ifdef HAVE_FALCON
...
int wc_PQPrivateKeyDecode(const byte* input, word32* inOutIdx, byte* key,
       					word32 inSz)
{
	int length;

	if (inOutIdx == NULL || input == NULL)
		return BAD_FUNC_ARG;

	if (GetOctetString(input, inOutIdx, &length, inSz) < 0)
		return ASN_PARSE_E;

	if (length != (CRYPTO_PUBLICKEYBYTES + CRYPTO_SECRETKEYBYTES))
		return ASN_PARSE_E;

	if (key == NULL){
		*inOutIdx += CRYPTO_PUBLICKEYBYTES;
	}
	else{
		XMEMCPY(key, input+(*inOutIdx), CRYPTO_SECRETKEYBYTES);
	}
...
static int GetKey(DecodedCert* cert)
{
...
#endif
    {
    	...
    	publicKey = (byte*)XMALLOC(CRYPTO_PUBLICKEYBYTES, cert->heap,
								   DYNAMIC_TYPE_PUBLIC_KEY);
		if (publicKey == NULL)
			return MEMORY_E;
		XMEMCPY(publicKey, &cert->source[cert->srcIdx], CRYPTO_PUBLICKEYBYTES);
		cert->publicKey = publicKey;
		cert->pubKeyStored = 1;
		cert->pubKeySize   = CRYPTO_PUBLICKEYBYTES;

		cert->srcIdx += CRYPTO_PUBLICKEYBYTES;
        ...
        case SIG_STATE_DO:
        {
            switch (keyOID) {
				...
				case DILITHIUM_LEVEL1k:
				{
					...
					ret = crypto_sign_verify(sig, sigSz, buf, bufSz, key);
					break;
				}
	#endif
	#ifdef HAVE_FALCON

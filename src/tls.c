...
/* ------------------------------------PQ FUNCTIONS------------------------------- */
#ifdef HAVE_POSTQUANTUM
/* Create a key share entry using post-quantum KEM algorithms (Kyber or Saber).
 * Generates a key pair, Public Key and Private Key (for client).
 *
 * ssl   The SSL/TLS object.
 * kse   The key share entry object.
 * returns 0 on success, otherwise failure.
 */
static int TLSX_KeyShare_GenPQKey(WOLFSSL *ssl, KeyShareEntry* kse)
{
    byte* pubKey = NULL;
    byte* privKey = NULL;
    word32 pubKeyLen, privKeyLen;
	int		ret;


    /* Map Public Key and Private Key lengths */
    ...
    if (pubKey == NULL) {
        WOLFSSL_MSG("PQ Private Key Memory error");
        return MEMORY_E;
    }

	/* Allocate space for the secret (private) key. */
    privKey = (byte*)XMALLOC(privKeyLen, ssl->heap, DYNAMIC_TYPE_PRIVATE_KEY);

    if (privKey == NULL) {
        WOLFSSL_MSG("PQ Public key Memory error");
        return MEMORY_E;
    }
	...
	/* Map PQ keys in KeyShareEntry */

	kse->pubKey = pubKey;
	kse->pubKeyLen = pubKeyLen;
	kse->key = privKey;
	kse->keyLen = privKeyLen;
...
/* Create a key share entry using post-quantum algorithms (Kyber or Saber).
 * Encapsulates the shared secret using peer's (client's) public key (for server).
 *
 * ssl   The SSL/TLS object.
 * kse   The key share entry object.
 * returns 0 on success, otherwise failure.
 */
static int TLSX_KeyShare_GenPQEncaps(WOLFSSL *ssl, KeyShareEntry* kse)
{
	byte* ciphertext;
	byte* sharedSecret;
	byte* pubKey;
	word32 ciphertextLen, sharedSecretLen;
	int 	ret;

	/* Map ciphertext and shared secret lengths */
	ciphertextLen = CRYPTO_CIPHERTEXTBYTES;
	sharedSecretLen = CRYPTO_BYTES;

	/* Allocate space for the ciphertext. */
	ciphertext = (byte*)XMALLOC(ciphertextLen, ssl->heap, DYNAMIC_TYPE_PUBLIC_KEY);

    if (ciphertext == NULL) {
        WOLFSSL_MSG("PQ Ciphertext Memory error");
        return MEMORY_E;
    }

	/* Allocate space for shared secret. */
    sharedSecret = (byte*)XMALLOC(sharedSecretLen, ssl->heap, DYNAMIC_TYPE_PRIVATE_KEY);

    if (sharedSecret == NULL) {
        WOLFSSL_MSG("PQ Shared Secret Memory error");
        return MEMORY_E;
    }
	 ...
	 *
	 * Ciphertext ----> kse->pubKey
	 * Shared Secret -> kse->key
	 */
	kse->key = sharedSecret;
	kse->keyLen = sharedSecretLen;
	kse->pubKey = ciphertext;
	kse->pubKeyLen = ciphertextLen;
	...
	/* Return */
		...
		if (ret != 0){
			/* Data owned by key share entry otherwise. */
			if (ciphertext != NULL)
				XFREE(ciphertext, ssl->heap, DYNAMIC_TYPE_PUBLIC_KEY);
			if (sharedSecret != NULL)
				XFREE(sharedSecret, ssl->heap, DYNAMIC_TYPE_PRIVATE_KEY);
		}
...
#ifdef HAVE_POSTQUANTUM
	#ifdef HAVE_KYBER
		if (kse->group == WOLFSSL_KYBER_LEVEL1 && ssl->options.side == WOLFSSL_CLIENT_END)
			return TLSX_KeyShare_GenPQKey(ssl, kse);
		if (kse->group == WOLFSSL_KYBER_LEVEL1 && ssl->options.side == WOLFSSL_SERVER_END)
			return TLSX_KeyShare_GenPQEncaps(ssl, kse);

		if (kse->group == WOLFSSL_KYBER_LEVEL3 && ssl->options.side == WOLFSSL_CLIENT_END)
			return TLSX_KeyShare_GenPQKey(ssl, kse);
		if (kse->group == WOLFSSL_KYBER_LEVEL3 && ssl->options.side == WOLFSSL_SERVER_END)
			return TLSX_KeyShare_GenPQEncaps(ssl, kse);

		if (kse->group == WOLFSSL_KYBER_LEVEL5 && ssl->options.side == WOLFSSL_CLIENT_END)
			return TLSX_KeyShare_GenPQKey(ssl, kse);
		if (kse->group == WOLFSSL_KYBER_LEVEL5 && ssl->options.side == WOLFSSL_SERVER_END)
			return TLSX_KeyShare_GenPQEncaps(ssl, kse);
	#endif
	#ifdef HAVE_SABER
		if (kse->group == WOLFSSL_SABER_LEVEL1 && ssl->options.side == WOLFSSL_CLIENT_END)
			return TLSX_KeyShare_GenPQKey(ssl, kse);
		if (kse->group == WOLFSSL_SABER_LEVEL1 && ssl->options.side == WOLFSSL_SERVER_END)
			return TLSX_KeyShare_GenPQEncaps(ssl, kse);

		if (kse->group == WOLFSSL_SABER_LEVEL3 && ssl->options.side == WOLFSSL_CLIENT_END)
			return TLSX_KeyShare_GenPQKey(ssl, kse);
		if (kse->group == WOLFSSL_SABER_LEVEL3 && ssl->options.side == WOLFSSL_SERVER_END)
			return TLSX_KeyShare_GenPQEncaps(ssl, kse);

		if (kse->group == WOLFSSL_SABER_LEVEL5 && ssl->options.side == WOLFSSL_CLIENT_END)
			return TLSX_KeyShare_GenPQKey(ssl, kse);
...
static void TLSX_KeyShare_FreeAll(KeyShareEntry* list, void* heap)
{
    ...
    while ((current = list) != NULL) {
        ...
        if ((current->group & NAMED_DH_MASK) == 0) {
			...
			else if (current->group == WOLFSSL_KYBER_LEVEL1){
				XFREE(current->key, heap, DYNAMIC_TYPE_PRIVATE_KEY);
				current->key = NULL;
			}
			else if (current->group == WOLFSSL_KYBER_LEVEL3){
				XFREE(current->key, heap, DYNAMIC_TYPE_PRIVATE_KEY);
				current->key = NULL;
			}
			else if (current->group == WOLFSSL_KYBER_LEVEL5) {
				XFREE(current->key, heap, DYNAMIC_TYPE_PRIVATE_KEY);
				current->key = NULL;
			}
	#endif
	#ifdef HAVE_SABER
			else if (current->group == WOLFSSL_SABER_LEVEL1){
				XFREE(current->key, heap, DYNAMIC_TYPE_PRIVATE_KEY);
				current->key = NULL;
			}
			else if (current->group == WOLFSSL_SABER_LEVEL3){
				XFREE(current->key, heap, DYNAMIC_TYPE_PRIVATE_KEY);
				current->key = NULL;
			}
			else if (current->group == WOLFSSL_SABER_LEVEL5){
				XFREE(current->key, heap, DYNAMIC_TYPE_PRIVATE_KEY);
				current->key = NULL;
			}
	#endif
...
/* Process the PQ KEM key share extension on the server side (map the shared secret).
 *
 * ssl            The SSL/TLS object.
 * keyShareEntry  The key share entry object to use to calculate shared secret.
 * returns 0 on success and other values indicate failure.
 */
static int TLSX_KeyShare_ProcessPQServer(WOLFSSL* ssl, KeyShareEntry* kse)
{
	word32 sharedSecretLen = 0;
	byte* sharedSecret = NULL;

	/* Map length and shared secret */
	sharedSecretLen = CRYPTO_BYTES;
	sharedSecret = kse->key;


	/* Copy shared secret in WOLFSSL */
	ssl->arrays->preMasterSz = sharedSecretLen;
...
/* Process the PQ KEM key share extension on the client side (decapsulate the shared secret).
 *
 * ssl            The SSL/TLS object.
 * keyShareEntry  The key share entry object to use to calculate shared secret.
 * returns 0 on success and other values indicate failure.
 */
static int TLSX_KeyShare_ProcessPQClient(WOLFSSL* ssl, KeyShareEntry* kse)
{
	byte* sharedSecret = NULL;
	...
	/* Map ciphertext and private key */
	ciphertext = kse->ke;
	privateKey = kse->key;
	...
	/* Copy the shared secret in WOLFSSL */
	ssl->arrays->preMasterSz = sharedSecretLen;
	XMEMCPY(ssl->arrays->preMasterSecret, sharedSecret, sharedSecretLen);
	XFREE(sharedSecret, ssl->heap, DYNAMIC_TYPE_PRIVATE_KEY);
...
#ifdef HAVE_POSTQUANTUM
	#ifdef HAVE_KYBER
		if (keyShareEntry->group == WOLFSSL_KYBER_LEVEL1 && ssl->options.side == WOLFSSL_SERVER_END)
			ret = TLSX_KeyShare_ProcessPQServer(ssl, keyShareEntry);
		if (keyShareEntry->group == WOLFSSL_KYBER_LEVEL1 && ssl->options.side == WOLFSSL_CLIENT_END)
			ret = TLSX_KeyShare_ProcessPQClient(ssl, keyShareEntry);

		if (keyShareEntry->group == WOLFSSL_KYBER_LEVEL3 && ssl->options.side == WOLFSSL_SERVER_END)
			ret = TLSX_KeyShare_ProcessPQServer(ssl, keyShareEntry);
		if (keyShareEntry->group == WOLFSSL_KYBER_LEVEL3 && ssl->options.side == WOLFSSL_CLIENT_END)
			ret = TLSX_KeyShare_ProcessPQClient(ssl, keyShareEntry);

		if (keyShareEntry->group == WOLFSSL_KYBER_LEVEL5 && ssl->options.side == WOLFSSL_SERVER_END)
			ret = TLSX_KeyShare_ProcessPQServer(ssl, keyShareEntry);
		if (keyShareEntry->group == WOLFSSL_KYBER_LEVEL5 && ssl->options.side == WOLFSSL_CLIENT_END)
			ret = TLSX_KeyShare_ProcessPQClient(ssl, keyShareEntry);
		return ret;
	#endif
	#ifdef HAVE_SABER
		if (keyShareEntry->group == WOLFSSL_SABER_LEVEL1 && ssl->options.side == WOLFSSL_SERVER_END)
			ret = TLSX_KeyShare_ProcessPQServer(ssl, keyShareEntry);
		if (keyShareEntry->group == WOLFSSL_SABER_LEVEL1 && ssl->options.side == WOLFSSL_CLIENT_END)
			ret = TLSX_KeyShare_ProcessPQClient(ssl, keyShareEntry);

		if (keyShareEntry->group == WOLFSSL_SABER_LEVEL3 && ssl->options.side == WOLFSSL_SERVER_END)
			ret = TLSX_KeyShare_ProcessPQServer(ssl, keyShareEntry);
		if (keyShareEntry->group == WOLFSSL_SABER_LEVEL3 && ssl->options.side == WOLFSSL_CLIENT_END)
			ret = TLSX_KeyShare_ProcessPQClient(ssl, keyShareEntry);

		if (keyShareEntry->group == WOLFSSL_SABER_LEVEL5 && ssl->options.side == WOLFSSL_SERVER_END)
			ret = TLSX_KeyShare_ProcessPQServer(ssl, keyShareEntry);
		if (keyShareEntry->group == WOLFSSL_SABER_LEVEL5 && ssl->options.side == WOLFSSL_CLIENT_END)
			ret = TLSX_KeyShare_ProcessPQClient(ssl, keyShareEntry);
		return ret;
...
int TLSX_KeyShare_Establish(WOLFSSL *ssl)
{
    ...
    for (clientKSE = list; clientKSE != NULL; clientKSE = clientKSE->next) {
        ...
        /* Bypass DH_mask checks for post-quantum */
